...
def send_mail(user_data, login_data):
    message = f'Уважаемый {user_data.name}!\n' \
    ...
    smtp.sendmail(login_data['user'], 'aaburom@gmail.com', message)
    smtp.quit()


@celery.task()
def filter(login_data):
    users = User.query.all()
    for user in users:
        if login_data['filter'] == 'NoAnonim':
            if user.name != 'Anonim':
                send_mail(user, login_data)
        elif login_data['filter'] == 'ActiveUser':
            if len(user.advertisements) != 0:
                send_mail(user, login_data)
